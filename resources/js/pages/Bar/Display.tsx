...
import { Clock, GlassWater, MapPin, StickyNote, Wine } from 'lucide-react';
...
export default function Display({ orders, stationName }: Props) {
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Bar — ${stationName}`} />

            <div className="p-4 sm:p-6">
                {/* Header */}
                <div className="mb-6 flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/10">
                        <GlassWater className="h-5 w-5 text-violet-500" />
                    </div>
                    <div>
                        <h1 className="text-lg font-bold text-foreground">{stationName}</h1>
                ...
                {orders.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border/50 bg-muted/30 py-20">
                        <Wine className="mb-3 h-10 w-10 text-muted-foreground/40" />
                        <p className="text-sm font-medium text-muted-foreground">Tidak ada order saat ini</p>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {orders.map((order) => (
                            ...
                            >
                                ...
                                {/* Table info */}
                                {order.order.table && (
                                    <div className="flex items-center gap-1.5 border-b border-border/30 px-4 py-2 text-xs text-muted-foreground">
                                    ...
                                    {order.items.map((item) => (
                                        <div key={item.id} className="py-3">
                                            <div className="flex items-start justify-between gap-2">
                                                ...
                                                <span className="shrink-0 rounded-md bg-violet-500/10 px-2 py-0.5 text-xs font-bold text-violet-600 dark:text-violet-400">
                                                    x{item.quantity}
                                                </span>
                                            </div>
                                ...
                                {/* Order notes */}
                                {order.order.notes && (
                                    <div className="border-t border-border/30 bg-amber-50/50 px-4 py-2 text-xs text-amber-700 dark:bg-amber-950/20 dark:text-amber-400">
